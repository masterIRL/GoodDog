...
public class FrameFichier  extends JFrame{
	...
	private void initialisation() {
		...
		lancerRecherche.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				//r�cup�re le type de fichier
				if(texteChemin.getText().length() >= 3) {
					String extension = texteChemin.getText().substring(texteChemin.getText().length()-3); //r�cup�re les 3 dernieres lettre
					...
					System.out.println("Choisissez un fichier"); //Le faire apparaitre � l'�cran
	...
	public void rechercheFichier() {
		if(typeFichier != null)
		{
			...
			System.out.println("Choisissez un fichier valide"); // Le faire apparaitre a l'�cran grace a un label etc
			boxErreur.setVisible(true);
		}
